...
def select_data(XX, YY, ctype, min_samples, output_folder):
    ...
    if ctype == 'diagnostic':
        ...
        # 1. Count all superdiagnnostic-labels
        counts = pd.Series(np.concatenate(YY.superdiagnostic.values)).value_counts()
        # 2. Filter out labels with less samples than min_samples
        counts = counts[counts > min_samples]
        # 3. Save only labels with enough samples
        YY.superdiagnostic = YY.superdiagnostic.apply(lambda x: list(set(x).intersection(set(counts.index.values))))
        # 4. Calculate new lenght of superdiagnostic labels
        YY['superdiagnostic_len'] = YY.superdiagnostic.apply(lambda x: len(x))
        # 5. Select only samples with minimum one superdiagnostic label
        X = XX[YY.superdiagnostic_len > 0]
        Y = YY[YY.superdiagnostic_len > 0]
        # 6. Create MultiLabelBinarizer and fit it to the superdiagnostic labels
