...
class PRCABasisProducer(EigenBasisProducer):
    '''
    Die Klasse PRCABasisProducer (ab Zeile ~150) ist der Kern für PRCA im Repo.
    Sie definiert die Methode, wie die PRCA-Basis (also die relevanten Komponenten/Subräume) aus Aktivierungs- und Kontextvektoren berechnet wird.
    '''
    def _construct_matrix(self, base_dir):
        '''
        Lädt die gespeicherten Aktivierungsvektoren (act.npy) und Kontextvektoren (ctx.npy).
        Sie berechnet daraus die PRCA-Matrix (mathematisch eine Mischung der beiden, siehe Paper).
        Dann werden daraus mittels Eigenzerlegung (SVD) die wichtigsten Richtungen/Komponenten gefunden.
        '''
