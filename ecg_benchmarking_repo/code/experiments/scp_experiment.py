...
class SCP_Experiment():
    ...
    def prepare(self):
        # Load PTB-XL data
        self.data, self.raw_labels = utils_edited.load_dataset(self.datafolder, self.sampling_frequency)

        # Preprocess label data
        self.labels = utils_edited.compute_label_aggregations(self.raw_labels, self.datafolder, self.task)

        # Select relevant data and convert to one-hot
        self.data, self.labels, self.Y, _ = utils_edited.select_data(self.data, self.labels, self.task, self.min_samples, self.outputfolder+self.experiment_name+'/data/')
        ...
        self.X_train, self.X_val, self.X_test = utils_edited.preprocess_signals(self.X_train, self.X_val, self.X_test, self.outputfolder+self.experiment_name+'/data/')
    ...
    def evaluate(self, n_bootstraping_samples=100, n_jobs=20, bootstrap_eval=False, dumped_bootstraps=True):
        ...
        if bootstrap_eval:
            if not dumped_bootstraps:
                ...
                test_samples = np.array(utils_edited.get_appropriate_bootstrap_samples(y_test, n_bootstraping_samples))
        ...
        for m in sorted(os.listdir(self.outputfolder+self.experiment_name+'/models')):
            ...
            te_df = pd.concat(pool.starmap(utils_edited.generate_results, zip(test_samples, repeat(y_test), repeat(y_test_pred), repeat(thresholds))))
            te_df_point = utils_edited.generate_results(range(len(y_test)), y_test, y_test_pred, thresholds)
